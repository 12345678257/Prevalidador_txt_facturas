...
def convert_excel_to_txt(df, codigo_factura="98353"):
    ...
    # Procesar cada fila (sin incluir el encabezado si existe)
    for idx, row in df.iterrows():
        # Obtener valores de las columnas (empezando desde la columna 3)
        values = []
        
        # Campo 1: Código de factura (puede venir del Excel o usar el valor por defecto)
        col3_value = format_value(row.iloc[3])
        codigo = codigo_factura if col3_value == "" else col3_value
        ...
        # Campos 9-20: Valores numéricos
        for i in range(11, 22):
            values.append(format_value(row.iloc[i]))
        
        # Campo 21: Fecha (formato DD/MM/YYYY)
        values.append(format_date(row.iloc[22]))
        
        # Campo 22: Descripción del procedimiento
        values.append(format_value(row.iloc[23]))
        
        # Crear contenido del archivo TXT
        txt_content = ",".join(values)
        
        # Nombre del archivo: RS_{codigo}_Archivo_DET_{numero_secuencial}.txt
        # El número secuencial evita que se sobrescriban archivos con el mismo código
        filename = f"RS_{codigo}_Archivo_DET_{idx+1}.txt"
